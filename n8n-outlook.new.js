...
for (const item of $input.all()) {
  ...
  try {
    const parentMsg = $input.item.json;
    if (parentMsg && parentMsg.receivedDateTime) rawDate = parentMsg.receivedDateTime;
  } catch (_) {}
  ...
  for (const att of attachments) {
    ...
    if (isPdf && att.contentBytes) {
      const fileName = name || ('invoice_' + Date.now() + '.pdf');
      outputItems.push({
        json: {
          relDir: relDir,
          fileName: fileName,
    ...
    return {
        ...
        nodes: [
            ...
            // FredFeuer Pipeline
            {
                parameters: {
                    method: 'GET',
                    url: `https://graph.microsoft.com/v1.0/me/mailFolders/${encodeURIComponent(folderIds['FredFeuer'])}/messages?$top=50&$filter=hasAttachments eq true&$select=id,receivedDateTime,subject,hasAttachments`,
                    authentication: 'predefinedCredentialType',
                    nodeCredentialType: 'microsoftOutlookOAuth2Api',
                    options: {},
                },
            ...
            },
            {
                ...
                name: 'Get Attachments (FredFeuer)', type: 'n8n-nodes-base.httpRequest', typeVersion: 4, position: [850, 160],
                credentials: { microsoftOutlookOAuth2Api: { id: OUTLOOK_CREDENTIAL_ID } },
            },
            {
                parameters: { mode: 'runOnceForAllItems', jsCode: makeSaveCode('FredFeuer'), options: {} },
                name: 'Prepare Path (FredFeuer)', type: 'n8n-nodes-base.code', typeVersion: 2, position: [1050, 160],
            },
            ...
            // Waschbeckenmanufaktur Pipeline
            {
                parameters: {
                    method: 'GET',
                    url: `https://graph.microsoft.com/v1.0/me/mailFolders/${encodeURIComponent(folderIds['Waschbeckenmanufaktur'])}/messages?$top=50&$filter=hasAttachments eq true&$select=id,receivedDateTime,subject,hasAttachments`,
                    authentication: 'predefinedCredentialType',
                    nodeCredentialType: 'microsoftOutlookOAuth2Api',
                    options: {},
                },
            ...
            },
            {
                ...
                name: 'Get Attachments (Waschbeckenmanufaktur)', type: 'n8n-nodes-base.httpRequest', typeVersion: 4, position: [850, 460],
                credentials: { microsoftOutlookOAuth2Api: { id: OUTLOOK_CREDENTIAL_ID } },
            },
            {
                parameters: { mode: 'runOnceForAllItems', jsCode: makeSaveCode('Waschbeckenmanufaktur'), options: {} },
                name: 'Prepare Path (Waschbeckenmanufaktur)', type: 'n8n-nodes-base.code', typeVersion: 2, position: [1050, 460],
            },
        ],
        connections: {
            'Webhook Trigger': {
            ...
            'Split Messages (FredFeuer)': { main: [[{ node: 'Get Attachments (FredFeuer)', type: 'main', index: 0 }]] },
            ...
            'Split Messages (Waschbeckenmanufaktur)': { main: [[{ node: 'Get Attachments (Waschbeckenmanufaktur)', type: 'main', index: 0 }]] },
...
function saveInvoicesFromExecution(exec) {
    ...
    console.log('\n─────────────────────────────────────────');
    console.log('📊  Summary:');
    console.log(`    📥  Fetched: ${totalFetched} email(s) with attachments`);
